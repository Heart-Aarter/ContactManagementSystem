...
bool inputCurrentUserPassword(const char* pLoginName, char* pPwd, int nPwdSize) {
    int nIndex = -1;

    if (pLoginName == nullptr || pLoginName[0] == '\0' || pPwd == nullptr || nPwdSize <= 0) {
        cout << "当前登录信息无效，请重新登录。" << endl;
        finishPage();
        return false;
    }

    cout << "当前账号: " << pLoginName << endl;
    getPwdOnce(pPwd);
    if (!validatePassword(pPwd)) {
        return false;
    }

    if (checkAccount(pLoginName, pPwd, &nIndex) == nullptr) {
...
void userLogon(const char* pLoginName) {
    showPageHeader("开始服务");
    ...
    if (!inputCurrentUserPassword(pLoginName, aPwd, sizeof(aPwd))) {
        return;
    }

    const int nResult = startServiceInfo(pLoginName, aPwd, &info);
    ...
    switch (nResult) {
        ...
        case TRUE:
            ...
            cout << "下一步: 服务结束后返回用户菜单选择 2 结束服务。" << endl;
            break;
        case UNUSE:
...
void userSettle(const char* pLoginName) {
    showPageHeader("结束服务");
    ...
    const int nResult = endServiceInfo(pLoginName, aPwd, &info);
...
void userAddMoney(const char* pLoginName) {
    showPageHeader("获取积分");
    ...
    const int nResult = doAddPointInfo(pLoginName, aPwd, &pointInfo);
...
void userRefundMoney(const char* pLoginName) {
    showPageHeader("使用积分");
    ...
    pointInfo.fChange = fAmount;
    const int nResult = doRefundPointInfo(pLoginName, aPwd, &pointInfo);
...
void userDonate(const char* pLoginName) {
    showPageHeader("物资捐赠");
    ...
    const int nResult = doDonatePointInfo(pLoginName, aPwd, &pointInfo);
...
bool userAnnul(const char* pLoginName) {
    showPageHeader("注销本人账号");

    Account account{};

    if (!inputCurrentUserPassword(pLoginName, account.aPwd, sizeof(account.aPwd))) {
        return false;
    }

    strncpy(account.aName, pLoginName, sizeof(account.aName) - 1);

    if (!confirmAction("确认注销当前登录账号")) {
...
void userServiceRecords(const char* pLoginName) {
    showPageHeader("本人服务记录查询");

    Tracking* pTracking = nullptr;
    int nCount = 0;

    pTracking = queryTrackingInfoByAccount(pLoginName, &nCount);
    cout << endl;
    if (pTracking == nullptr || nCount == 0) {
        printEmptyHint("当前没有本人的服务记录。", "可先完成一次开始服务和结束服务流程。");
        finishPage();
        return;
    }
...
void userPointRecords(const char* pLoginName) {
    showPageHeader("本人积分流水查询");

    PointChange* pPoints = nullptr;
    int nCount = 0;

    pPoints = queryPointChangeInfoByAccount(pLoginName, &nCount);
    cout << endl;
    if (pPoints == nullptr || nCount == 0) {
        printEmptyHint("当前没有本人的积分流水。", "可先获取积分、使用积分或完成物资捐赠。");
        finishPage();
        return;
    }
...
void userMainMenu(const char* pLoginName) {
    ...
    do {
        ...
        cout << "9. 使用帮助/规则说明" << endl;
        cout << "0. 退出登录" << endl << endl;

        if (readMenuSelection("请选择用户功能(0~9): ", 0, 9, &nSelection)) {
            switch (nSelection) {
                case 1:
                    userLogon(pLoginName);
                    break;
                case 2:
                    userSettle(pLoginName);
                    break;
                case 3:
                    userAddMoney(pLoginName);
                    break;
                case 4:
                    userRefundMoney(pLoginName);
                    break;
                case 5:
                    userDonate(pLoginName);
                    break;
                case 6:
                    if (userAnnul(pLoginName)) {
                        nSelection = 0;
                    }
                    break;
                case 7:
                    userServiceRecords(pLoginName);
                    break;
                case 8:
                    userPointRecords(pLoginName);
                    break;
                case 9:
                    helpPage();
                    break;
